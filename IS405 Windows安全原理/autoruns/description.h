...
void FileDescription(TCHAR* filePath, QString& res){
    TCHAR* buf = new TCHAR[1024];
    if ( GetFileVersionString(filePath,_T("FileDescription"), buf, 1024)){
        res = QString::fromWCharArray(buf);
    }
    else{
        res = "";
    }
...
void DllDescription (TCHAR* filePath, QString& res){
    TCHAR* buf = new TCHAR[1024];
    if (SHLoadIndirectString(filePath, buf, 1024, NULL) == S_OK)
            res = QString::fromWCharArray(buf);
        else
            res = "";
    delete[] buf;
